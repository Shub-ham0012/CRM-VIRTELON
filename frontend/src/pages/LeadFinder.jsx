    ...
    <div className="fade-up">
      ...
      <div className="surface rounded-lg p-5">
        ...
        <div className="mt-5 flex items-center gap-3">
          ...
          <span className="text-xs text-zinc-500 flex items-center gap-1.5"><Sparkles className="h-3.5 w-3.5 text-[#3b82f6]" /> Provider abstraction ready for live Google Places / Search API</span>
        </div>
      </div>
      ...
      {provider && !provider.live && (
        <div className="mt-4 flex items-start gap-2 rounded-md bg-amber-400/8 border border-amber-400/20 px-4 py-3 text-sm text-amber-300/90">
          ...
          <span>These results are generated by the <strong>mock provider</strong> — realistic sample data, <strong>not live</strong>. Contact details are intentionally blank and never fabricated. Connect a real provider in Settings to go live.</span>
        </div>
      )}
              ...
              <div key={l.id} data-testid="finder-result-card" className="surface rounded-lg p-4">
                ...
                <div className="mt-3 flex flex-wrap gap-1.5 text-[11px] font-mono">
                  <span className={`rounded px-2 py-0.5 border ${l.website_status === "Missing" ? "text-red-400 border-red-400/20 bg-red-400/10" : "text-zinc-400 border-white/10 bg-white/5"}`}>
                    <Globe className="h-3 w-3 inline mr-1" />{l.website_status}
                  </span>
                  ...
                  {l.google_url && <span className="rounded px-2 py-0.5 border border-white/10 bg-white/5 text-zinc-400">Google</span>}
                </div>
